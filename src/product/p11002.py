...
class P11002(Generate):
    ...
    def strategy_process(self):
        ...
        try:
            ...
            logger.info("1. 财务报表分析报告：获取策略引擎结果，流程开启, 入参为：%s", json.dumps(json_data))
            ...
            task_no = strategy_param.get('taskNo')
            ...
            for data in query_data_array:
                index = index + 1
                logger.info("P11002_process------------" + str(index) + "/" + str(total))
                resp = self.strategy(data, product_code, req_no,task_no,json_data)
                subject.append(resp)
    ...
    def strategy(self, data, product_code, req_no,task_no,json_data):
        ...
        data_repository = {'task_no':task_no,'product_code':product_code}
        variables, out_decision_code = translate_for_strategy(product_code, biz_types, user_name, id_card_no, phone,
                                                              user_type, base_type, origin_data=data, data_repository=data_repository,full_msg=json_data)
