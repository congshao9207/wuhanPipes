...
from product.eloan.eloan_amt_flow_executor import ELoanAmtFlowExecutor
from product.eloan.eloan_common_flow_executor import ELoanCommonFlowExecutor
...
class P09003(Generate):
    ...
    def shake_hand_process(self):
        try:
            ...
            # channelSource  快易宝业务来源 0-磁金融；1-中金同盛
            ...
            for data in query_data_array:
                response_array.append(self._shake_hand_response(base_type_service, data, product_code, req_no,channel_source))
            resp = {
                'appId': app_id,
    ...
    def find_process_flow(self, json_data, query_data_array):
        executor = None
        all_finished = len(list(filter(lambda x: x["nextSegmentName"] != "loan_amt" and x["nextSegmentName"] != '/', query_data_array))) == 0

        for item in query_data_array:
            segment_name = item.get("nextSegmentName")
            if segment_name == "loan_amt" and all_finished:
                executor = ELoanAmtFlowExecutor(json_data)
                break

        if not executor:
            executor = ELoanCommonFlowExecutor(json_data)

        executor.sql_db = self.sql_db
        executor.df_client = self.df_client
        return executor

    def _shake_hand_response(self, base_type_service, data, product_code, req_no,channel_source):
        """
        和决策交互，封装response
        :param data:
        :param product_code:
        ...
        variables['channel_source'] = channel_source
