...
from view.p08001_v.sunstreaker_feature import SunstreakerFeature
...
class P08001(Generate):
    ...
    def shake_hand_process(self):
        ...
        try:
            ...
            for data in query_data_array:
                ...
                # 实名版不再传递担保人流水到画像阶段
                if product_code == '08001' and 'GUAR' in str(data.get('relation')):
    ...
    def _query_entity_hand_shake(self, json_data, data, query_data_array):
        ...
        public_param = {
            ...
            "productCode": product_code,
            "isSingle": is_single,
            "outApplyNo": json_data.get("preReportReqNo"),  # 把outApplyNo 调整为preReportReqNo
    ...
    def strategy_process(self):
        ...
        try:
            ...
            # 处理节点，将营销反哺部分数据新增到strategyInputVariables下
            response_data = self.response.copy()
            ssf = SunstreakerFeature(response_data)
            self.response = ssf.process()
